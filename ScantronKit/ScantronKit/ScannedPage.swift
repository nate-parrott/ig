...
public class ScannedPage {
    ...
    lazy public var barcode: Barcode = {
        let x: CGFloat = CGFloat(PROPORTIONAL_COMB_MARGIN) * self.image.size.width
        let y: CGFloat = CGFloat(1.0 - PROPORTIONAL_BARCODE_HEIGHT) * self.image.size.height
        let w: CGFloat = CGFloat(1.0-PROPORTIONAL_COMB_MARGIN*2.0) * self.image.size.width
        let h: CGFloat = CGFloat(PROPORTIONAL_BARCODE_HEIGHT) * self.image.size.height
        ...
        let subImage = self.image.subImage(barcodeFrame)
